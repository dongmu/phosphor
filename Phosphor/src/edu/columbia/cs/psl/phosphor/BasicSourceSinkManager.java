...
public class BasicSourceSinkManager extends SourceSinkManager {
	...
	static {
		if (Instrumenter.sourcesFile == null
				&& Instrumenter.sinksFile == null
				&& !TaintTrackingClassVisitor.IS_RUNTIME_INST) {
			System.err.println("No taint sources or sinks specified. " 
					+ "To specify, add option -taintSources file and/or -taintSinks file "
					+ "where file is a file listing taint sources/sinks. "
					+ "See files taint-sinks and taint-samples in source for examples. "
		...
		if (!TaintTrackingClassVisitor.IS_RUNTIME_INST) {
			System.out.println("Loaded " + sinks.size()+ " sinks and "
	...
	private static BasicSourceSinkManager instance;
	...
	public boolean isSource(String str) {
		...
		try {
			...
			for (String s : sources) {
				String d[] = s.split("\\.");
				if (d[1].equals(inD[1])
						&& c1IsSuperforC2(d[0], inD[0])) { // desc is same
